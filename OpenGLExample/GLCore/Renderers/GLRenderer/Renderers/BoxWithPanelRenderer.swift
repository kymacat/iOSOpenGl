...
class BoxWithPanelRenderer: GLRenderer {
  ...
  override func glkViewControllerUpdate(_ controller: GLKViewController) {
    ...
    let mirrorView = GLKMatrix4(eye: [0, 2.5, 0.5], center: [0.0, 0.0, 0.0], up: [0.0, 0.0, 1.0], lhs: true)
    let mirrorProj = GLKMatrix4(projectionFov: .pi / 2, near: 1, far: 10, aspect: 1, lhs: true)
    drawBox(view: mirrorView, proj: mirrorProj)
    ...
    drawPanel(view: view, proj: proj)

    program.prepareToDraw()
    mesh.prepareToDraw()
    drawBox(view: view, proj: proj)
